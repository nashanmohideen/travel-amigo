...
interface GeneratedItineraryCardProps {
  ...
  dayIndex: number;
  /** When true, hides all edit controls (Remove, Replace, Move up/down) */
  readOnly?: boolean;
  callbacks?: EditCallbacks;
}

const categoryBadgeVariant: Partial<
  Record<PlaceCategory, "teal" | "amber" | "green" | "stone">
> = {
  nature: "green",
  culture: "amber",
  food: "amber",
...
const sessionConfig = {
  Morning:   { icon: "🌅", color: "text-amber-600",  bg: "bg-amber-50",  border: "border-amber-200" },
  Afternoon: { icon: "☀️",  color: "text-orange-600", bg: "bg-orange-50", border: "border-orange-200" },
  Evening:   { icon: "🌙", color: "text-indigo-600", bg: "bg-indigo-50", border: "border-indigo-200" },
};

// ── Local-transfer row shown between consecutive items ────────────────────────
  ...
  return (
    <div className="flex items-center gap-3 px-1 py-1">
      ...
      <div className="flex flex-col items-center shrink-0">
        ...
        <span className="text-[10px] text-stone-400 leading-none py-0.5">⬇</span>
        <div className="w-px h-3 bg-stone-200" />
      </div>
      <div className="flex items-center gap-1.5 rounded-lg bg-stone-50 border border-stone-100 px-3 py-1.5">
        <span className="text-xs">🚌</span>
        <span className="text-xs text-stone-500">~30 min local transfer</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="flex flex-col items-center shrink-0 pt-1.5">
        <div className="w-3 h-3 rounded-full border-2 border-teal-500 bg-white shadow-sm shrink-0" />
      </div>
      ...
      <div className="flex-1 min-w-0 pb-1">
        <div className="rounded-xl border border-stone-100 bg-stone-50 overflow-hidden">
          ...
          <div className="p-3">
            ...
            {/* Travel tip */}
            <div className="flex gap-1.5 rounded-lg bg-amber-50 border border-amber-100 px-2.5 py-2 mb-3">
            ...
            {/* ── Edit toolbar ─────────────────────────────────── */}
            {!readOnly && callbacks && (
            <div className="no-print flex flex-wrap items-center gap-1.5 pt-2 border-t border-stone-100">
              <button
              ...
              <div className="ml-auto flex items-center gap-1.5">
                <button
                  ...
                  className="flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium border border-teal-200 bg-teal-50 text-teal-700 hover:bg-teal-100 hover:border-teal-300 transition-colors active:bg-teal-200"
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium border border-red-200 bg-red-50 text-red-600 hover:bg-red-100 hover:border-red-300 transition-colors active:bg-red-200"
...
export default function GeneratedItineraryCard({ day, readOnly = false, callbacks }: GeneratedItineraryCardProps) {
  // Group items by session but track flat indices for move up/down
  ...
  return (
    <Card className="flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-teal-700 text-sm font-bold text-white shadow-sm">
              {day.day}
            </span>
          ...
          <h3 className="text-lg font-extrabold text-stone-900">{day.title}</h3>
        </div>
        {day.totalCostLkr > 0 && (
          <div className="text-right shrink-0">
            <p className="text-[10px] text-stone-400 uppercase tracking-wide">Est. day cost</p>
      ...
      {day.items.length > 0 ? (
            ...
            return (
              <div key={session.label}>
                ...
                <div className="flex items-center gap-2 mb-3 mt-1">
                  <div className={cn("flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold border", cfg.bg, cfg.color, cfg.border)}>
                    <span>{cfg.icon}</span>
                  ...
                  {session.items.map(({ item, flatIndex }, idx) => (
                    <div key={item.id}>
                      <PlaceItem
                        ...
                        callbacks={callbacks}
                      />
                      {/* Transfer row between items (not after last item of last session) */}
        ...
        <div className="rounded-xl bg-stone-50 border border-dashed border-stone-200 px-4 py-8 text-center">
      ...
      {/* ── Transport note ─────────────────────────────────────────── */}
      {day.transportNote && (
        <div className="flex gap-3 items-start rounded-xl bg-sky-50 border border-sky-100 px-4 py-3">
